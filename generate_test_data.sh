...
for day in {1..11}; do
    ...
    if [[ $day -le 3 ]]; then
        ...
        # Condition 9: Vérifier qu'une série ne peut pas être incrémentée deux fois le même jour
        echo "\"$timestamp\",\"2\",\"False\",\"True\",\"009\",\"$formatted_date\"" >> "$output_file"
        echo "\"$timestamp\",\"2\",\"True\",\"False\",\"009\",\"$formatted_date\"" >> "$output_file"
        # Répéter les exercices pour le même jour
        echo "\"$timestamp\",\"2\",\"False\",\"True\",\"009\",\"$formatted_date\"" >> "$output_file"
        echo "\"$timestamp\",\"2\",\"True\",\"False\",\"009\",\"$formatted_date\"" >> "$output_file"

        # Générer des données pour les tests du système de vie sur plusieurs jours
        echo "\"$timestamp\",\"2\",\"True\",\"True\",\"010\",\"$formatted_date\"" >> "$output_file"
